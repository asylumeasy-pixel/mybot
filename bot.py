...
def s5(m):
    ...
    user_data[m.from_user.id]["ВНЖ/2-е"] = m.text
...
def s8_usa(m):
    ...
    if "да" in m.text.lower():
        ...
        bot.send_message(m.chat.id, "*Как ты собираешься попасть в США?*", parse_mode="Markdown", reply_markup=markup)
        bot.register_next_step_handler(m, s9_usa)

def s9_usa(m):
    if m.text == "Назад": return s8_usa(m)
